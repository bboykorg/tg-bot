                    ...
                    if result:
                        ...
                        if energy <= energy_max:
                            ...
                            cursor.execute("UPDATE \"score\" SET \"energy\" = %s WHERE \"ID_user\" = %s",
                                       (new_energy, user_id))
                conn.commit()
        time.sleep(1.5)  # Обновлять каждую одну секунду
...
def index():
    ...
    return render_template("index.html", click_count=click_count, energy=energy, improvement=improvement,
                           current_energy=a_energy)
